    ...
    if lib.TLI_BuildDeviceList() == 0:
        ...
        while True:
            ...
            try:
                ...
                speed_response = requests.get(GET_SPEED_URL)
                stop_response = requests.get(GET_STOP_URL)
                # Check if the request was successful
                if response.status_code == 200 and speed_response.status_code == 200 and stop_response.status_code == 200:
                    # Parse the JSON response if applicable
                    data = response.json()
                    print("Position retrieved:", data)  # Replace 'your_key' with the actual key
                    speed = speed_response.json()
                    print("Speed retrieved:", speed)
                    stop =  stop_response.json()
                else:
                    print(f"Failed to retrieve data: {response.status_code} - {response.text}")
                    print(f"Failed to retrieve data: {speed_response.status_code} - {speed_response.text}")
                    continue
                
            except requests.RequestException as e:
                print(f"An error occurred: {e}")
            
            user_input = float(data) #not sure this is how it works, need to characterize the motor
            if user_input  < 0: 
                continue
            #set the new speed params -> user wanted a slower or faster step size:
            if(speed): 
                lib.CC_SetMotorParamsExt(serial_num, STEPS_PER_REV, gbox_ratio, pitch)
            else:
                lib.CC_SetMotorParamsExt(serial_num, STEPS_PER_REV_SLOW, gbox_ratio, pitch)

            # set a new position in device units
            new_pos_real = c_double(user_input)  # in real units
            ...
            if (real_pos.value != user_input):
                ...
                lib.CC_GetRealValueFromDeviceUnit(serial_num,
                                            ...
                                            byref(real_pos),
                                            0)
                while(real_pos.value != user_input):
                    time.sleep(0.5)
                    dev_pos2 = c_int(lib.CC_GetPosition(serial_num))
                    real_pos = c_double()
                    lib.CC_GetRealValueFromDeviceUnit(serial_num,
                                                dev_pos2,
                                                byref(real_pos),
